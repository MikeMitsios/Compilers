...
/**
   ...
   public String visit(MessageSend n, String argu) throws Exception {
      ...
      String parent;
        ...
        n.f4.accept(this, c_f[0]+" "+c_f[1]+" "+pr1+" "+fun);	//after all the ex are checked we see if the var_counter is the same with the num of parameters we want
        if(var_counter+1<par_num){		//means that the parameters ar more than the Expressions
   ...
   public String visit(ExpressionList n, String argu) throws Exception {
      String _ret=null;
      var_counter=0;			//a counter to show me whose parameter turn is to be checkd
      int par_num=0;
      ...
      String parent=null;
			if(symbolmap.containsKey(ex1)){
				...
				while(parent!=null){
					symClass p2=symbolmap.get(parent);
					if(pars.get(var_counter).equals(parent)){	//get the type of the parent that has the type we want and break
	  				break;
	  			}
					parent=p2.get_exname();
				}
      ...
      if(par_num<var_counter+1){		//means that the Expression list is bigger than the parameters of the function
      		throw new MyCustomException(c_f[0]+"===="+c_f[1]+"/"+"in function "+c_f[3]+" the "+var_counter+" parametre is not declared");
      }
      if(parent==null){
      		if(!pars.get(var_counter).equals(ex1)){		//check if the type is equal with the var_counter parameter
	      		throw new MyCustomException(c_f[0]+"===="+c_f[1]+"/"+"in function "+c_f[3]+" the "+var_counter+" parametre is not that "+ex1+" type");
	      }
      }
      else{
      		if(!pars.get(var_counter).equals(ex1) && !pars.get(var_counter).equals(parent)){		//check if the type is a parent of the type we use (if a type is a class)
	      		throw new MyCustomException(c_f[0]+"===="+c_f[1]+"/"+"in function "+c_f[3]+" the "+var_counter+" parametre is not that "+ex1+" type");
	     	 }
      }
    ...
    public String visit(ExpressionTerm n, String argu) throws Exception {
      ...
      String ex1=n.f1.accept(this, c_f[0]+" "+c_f[1]);
			...
			if(symbolmap.containsKey(ex1)){
				...
				while(parent!=null){
					symClass p2=symbolmap.get(parent);
					if(pars.get(var_counter).equals(parent)){	//get the type of the parent that has the type we want
	  				break;
	  			}
					parent=p2.get_exname();
				}
   ...
   public String visit(AllocationExpression n, String argu) throws Exception {
      ...
      // for(symClass p : symboltable){			//check if the ident is a class
      // 	if(p.get_cname().equals(cname)){
      // 		flag=1;
      // 		break;
      // 	}
      // }
